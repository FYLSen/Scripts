...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.discount = 0;
            for (let j = 0; j < LinkIdlist.length; j++) {
                LinkId = LinkIdlist[j];
...
function speedSignInDay() {
    ...
    return new Promise((resolve) => {
        $.post(signPostUrlh5st('apSignIn_day', data), (err, resp, data) => {
            try {
                if (err) {
...
function signPostUrl(function_id, body) {
    return {
        url: `${JD_API_HOST}`,
        body: `functionId=${function_id}&body=${JSON.stringify(body)}&appid=activities_platform&client=H5&clientVersion=1.0.0&t=${new Date().getTime()}`,
        headers: {
            "Host": "api.m.jd.com",
            "Content-Type": "application/x-www-form-urlencoded",
...
function signPostUrlh5st(function_id, body) {
    return {
        url: `${JD_API_HOST}`,
        body: `functionId=${function_id}&body=${JSON.stringify(body)}&appid=activities_platform&client=H5&clientVersion=1.0.0&t=${new Date().getTime()}&h5st=${decrypt(Date.now(), new Date().getTime(), '', JD_API_HOST)}`,
        headers: {
            "Host": "api.m.jd.com",
            "Content-Type": "application/x-www-form-urlencoded",
            "Origin": "https://daily-redpacket.jd.com",
